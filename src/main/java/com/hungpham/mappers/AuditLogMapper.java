package com.hungpham.mappers;

import com.hungpham.dtos.AuditLogDto;
import com.hungpham.entity.AuditLogEntity;
import org.mapstruct.*;

@Mapper(componentModel = "spring", uses = {UuidBinaryMapper.class, DateTimeMapper.class, UserRefMapper.class})
public interface AuditLogMapper {

    @Mappings({
            @Mapping(target = "id", source = "id"), // Long id
            @Mapping(target = "actorId", source = "actor", qualifiedByName = "userToId"),
            @Mapping(target = "entityId", source = "entityId", qualifiedByName = "toUuid"),
            @Mapping(target = "createdDate", source = "createdDate", qualifiedByName = "ldtToString")
    })
    AuditLogDto toDto(AuditLogEntity entity);

    @Mappings({
            @Mapping(target = "id", source = "id"),
            @Mapping(target = "actor", ignore = true), // set ở service
            @Mapping(target = "entityId", source = "entityId", qualifiedByName = "toBytes"),
            @Mapping(target = "createdDate", source = "createdDate", qualifiedByName = "stringToLdt")
    })
    AuditLogEntity toEntity(AuditLogDto dto);
}
